...
namespace CheesGame
{
    public class Board
    {
        ...
        /// <summary>
        /// It takes a player as parameter and return true if and only if that player's King is in check
        /// We will check if any of the opponent's pieces can capture the given player's King
        /// </summary>
        ...
        /// <summary>
        /// Copy the board to filter out any moves which leave the current player's King in check
        /// </summary>
        /// <returns></returns>
        public Board Copy()
        {
        ...
        private bool HasPawnInPosition(Player player, Position[] pawnPositions, Position skipPosition)
        {
            foreach (Position pos in pawnPositions.Where(IsInside))
            {
                ...
                EnPassant move =  new EnPassant(pos, skipPosition);
